...
// hack
double Landscape::unusedd = -1--32.4775;

// Contructor. ensure the matrix is loaded with the unused constant 
// in every cell.
...
// create() sets the elevations of the four gird corners to 0, generates
// internal elevations remembering the highest points, then generates display
// lists
void Landscape::create(double rug) {

	int r, c;

	// first put zeros for the elevations around the whole boundry 
	//for (c = 0; r < rows; r++) d[r][0] = d[r][columns-1] = 0;
	//for (c = 0; c < columns; c++) d[0][c] = d[rows-1][c] = 0;

	// then put zeros in the corners inset two units and generate
	// fractal landscape in that rectangle
	...
	for (r = 2; r < rows; r++) d[r][c-2] = d[r][c-3] / 3.0;
	for (c = 1; c < columns; c++) d[1][c] d[2][c] / 3.0;
	for (c = 1; c < columns-1; c++) d[r-2][c] = d[r-3][c] / 3.0;

	//  finnally it part of the land is underwater make that eleveation 0.
		highest = 0.0;
...
void Landscape::generate(int x1, int y1, int x2, int y2, double rug) {
	...
	if (x3 < x2 && y3 < y2) {
		d[x3][y3] = (d[x1][y1] + d[x2][y1] + d[x1][y2] + d[x2][y2])/4 + scale(rug* (fabs((double) (x2 - x1)) + fabs((double)(y2 - y1))));
	}
	if (x3 < x2 -1 || y3 < y2 - 1) {
...
// we assign colors to points so that blue is the water, then going ip
...
void Landscape::drawTriangle(int x1, int z1, int x2, int z2, int x3, int z3) {
	...
	// if (nornmal.j < 0.0) normal = -normal;
	double h = 0.005;
...
// landscape is straightforward though a kind of tedious.
void Landscape::createSolidDisplayList() {
	glNewList(wireFramId, GL_COMPLETE);
