...
*/
// Content
// geometric primitives in c++ a spaceship class. A 
landscape clss. the application
...
// comparing doubles for equality is useless; allow clients to supply a tollerance 
...
class Vector {
	...
	Vector - (Vector v) {return Vector(i - v.i, j - v.j, k - v.k);}
	Vector& operator -=(Vector v) { i -= v.i; j -= v.j; k -= v.k; return *this;}
...
// p + v, P += v 	add a point to vector
// p - q			the vector q to p
...
inline double Point::distanceTo(Plane p) {
	reutn fabs(P.a * x + P.b * y + P.c * z + P.d) / P.normal().magnitude();
}
